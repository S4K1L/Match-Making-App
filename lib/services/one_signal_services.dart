...
class OneSignalHelper {
  ...
  static Future<void> initialize() async {
    ...
    if (AppConstants.onesignalAppId.isEmpty ||
        AppConstants.onesignalAppId == 'token') {
  ...
  static Future<void> onUserLogin({
    ...
    await syncDeviceToBackend(userId: userId);
  }
  ...
  static Future<String> syncDeviceToBackend({required String userId}) async {
    if (!_initialized) return 'onesignal_not_initialized';

    try {
      final payload = {
        'user_id': userId,
        'onesignal_id': await getPlayerId(),
        'subscription_id': await getSubscriptionId(),
        'push_token': OneSignal.User.pushSubscription.token,
        'opted_in': OneSignal.User.pushSubscription.optedIn,
      };

      final response = await _api.post(
        ApiConstant.oneSignalDeviceSync,
        payload,
        authReq: true,
      );
  ...
  static Future<String> sendOfflineCallPush({
    ...
    try {
      ...
      debugPrint('Offline call push error: $e');
      return 'failed';
    }
  }
